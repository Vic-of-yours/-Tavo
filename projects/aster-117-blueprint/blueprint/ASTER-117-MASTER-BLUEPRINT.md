...
> 结论先行：**86 个可执行模块是待共同校准的候选值：核心 20、功能 28、图鉴 12、系统 18、皮肤 8。** 另候选保留 25 个默认禁用的叙事源分块；预设、TPG 宿主和静态素材包均不计入五书模块数。

本文件现作为 `117-to-aster-blueprint-r002` 的校准候选蓝图，不代表用户已经批准 86 模块。当前可执行源码仍是经定向清理后的 Aster 36 模块，不把规划中的 86 模块冒充已实现。r002 已完成代码包/素材包边界、2,667 个来源文件表示的逐项处置、精确内容去重门、94 个视觉素材的用途分类/短名化和人物位图清理：三个角色语义槽保留但默认空，基础包只提供非人物的 CSS 降级层；真正人物内容只能由世界 Mod 或用户资源显式注入。
...
素材清理已在 r002 执行：经尺寸、透明度、引用关系与人工复核确认的四份人物位图没有进入最新工作树，相关默认值改为空槽；94 个视觉文件按用途短名化，原/新路径与 SHA-256 见 `assets/catalog.json`。后续模块退役仍不得误删其唯一素材。
